...
after   → what the reader can explain, predict, or choose
...
one spine, narrow or split them. If the input is broad but coherent, choose a boundary and continue.
...
Each section must answer a necessary subquestion. Give it a question, answer, prerequisite, role, boundary, and next
question. The order follows prerequisite or causal dependency where one exists; parallel or composable sections state
their relationship instead of pretending one causes the next.

Name sections after the question or decision they resolve. Use one title convention: either the filename is the implicit
title and major chapters are H1 siblings, or the first H1 is the explicit title and chapters are H2. Never jump heading
levels.

Use one running example only when it reduces abstraction. A fact backed by a source but lacking a role in the spine is
deferred or dropped.
...
make a transition. If it has no job in the spine, merge, move, defer, or delete it.
...
Read the note top to bottom without following links. Stop when:
...
State the spine and each top-level section's role in one sentence. If that cannot be done from the note alone, repair the
structure before polishing prose.
